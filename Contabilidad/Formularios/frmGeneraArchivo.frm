...
'
'End Sub
Private Sub GeneraArchivoRegulatorio(strCodRegistro As String, strCodFondo As String, strCodAdministradora As String, datFechaReporte As Date)
    ...
    numNumRegistroControl = GeneraNumRegistroControl(strCodRegistro, gstrCodAdministradora, strFechaRegistro)
    
    numOut = FreeFile
    Open strNombreArchivo For Binary Access Read Write As numOut
    
    'Open strNombreArchivo For Output As #1

    'REGISTRO DE CONTROL
    strRegistro = ""
    strRegistro = strRegistro + "C"
    strRegistro = strRegistro + Format(Trim(strCodAdministradoraCNS), "@@@@@@")
    strRegistro = strRegistro + Trim(strCodRegistro)
    ...
    strRegistro = strRegistro + Format(Left(" ", 15), "@@@@@@@@@@@@@@@!")
...
'    End If
    ...
    With adoComm
        ...
        .CommandText = "{call up_CNProcArchivosRegulatoriosCNSV ('" & strCodRegistro & "','" & strCodFondo & "','" & gstrCodAdministradora & "','" & strFechaRegistro & "')}"
        
        Set adoRegistro = .Execute

        If Not adoRegistro.EOF Then
            numCampos = adoRegistro.Fields(0).Value
        End If

        Do While Not adoRegistro.EOF
            
            For n = 1 To numCampos + 1
...
Public Sub Buscar()
    ...
    Set adoRegistro = New ADODB.Recordset
    strSQL = "SELECT * FROM ArchivoRegulatorio WHERE CodAdministradora='" & gstrCodAdministradora & "' AND EstadoArchivo = '01'"
...
Private Sub GenerarArchivos(FECHA As Date, File As String)
    ...
    '----------------------------------------------------------
    strNombreArchivo = RptPath + File + "_" + dd_File + mm_File + "." + EXT_File
    
    Call GeneraArchivoRegulatorio(File, strCodFondo, gstrCodAdministradora, FECHA)
